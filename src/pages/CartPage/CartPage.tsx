...
const CartPage = () => {
  ...
  const clearCart = () => {
    const check = window.confirm('Чи ви впевнені, що хочете видалити всі предмети з корзини?');
    if (check) {
      dispatch(clearItems());
    }
  };
      ...
      {items.length > 0 ? (
        <div className="cartpage">
          ...
          <div className="cartpage__info">
            <div className="cartpage__info-count">
              Всього піц: <span>{totalCount} шт.</span>
            </div>
            <div className="cartpage__info-price">
              Сума замовлення: <span>{totalPrice} $</span>
            </div>
          </div>
          <div className="cartpage__bottom">
            <Link to="/" className="cartpage__bottom-back">
              Повернутися назад
            </Link>
            <button className="cartpage__bottom-pay">Оплатити зараз</button>
          </div>
        </div>
      ) : (
        <div className="cartpage__empty">
          <h2>Наразі корзина пуста. Замовте вже щось!</h2>
          <Link to="/" className="cartpage__bottom-back">
            Повернутися назад
          </Link>
        </div>
